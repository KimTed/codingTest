...
const failSolution = (arr, cnt) => {
   // 배열안에 음수가 들어갈 경우 실패!!!
   const arrLength = arr.length;
   const pivotStr = arr.join("").repeat(2);
   const startPoint = arrLength - parseInt(cnt % arrLength);
   return Array.from(
      pivotStr.substr(startPoint, arrLength)
      , x => parseInt(x));
}

const solution = (arr, cnt) => {
   const arrLength = arr.length;
   const start = arrLength - parseInt(cnt % arrLength);
   const end = start + arrLength;
   const pivotArr = arr.concat(arr);

   return pivotArr.slice(start, end);
}

(() => {
   try {
      console.log(solution([3, 8, 9, 7, 6], 3)); // [9, 7, 6, 3, 8]
      console.log(solution([3], 0));             // [3]
   } catch(err) {
      console.error(err);
   }
})();